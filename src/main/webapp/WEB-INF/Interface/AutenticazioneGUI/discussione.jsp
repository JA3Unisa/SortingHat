...
<head>
    <jsp:include page="../partials/headS.jsp">
        <jsp:param name="userStyles" value="main,background,discussione"/>
        <jsp:param name="userScripts" value="mobileMenu"/>
        <jsp:param name="title" value="SortingHat - Discussione Forum"/>
    </jsp:include>
</head>
<body>
...
<div id="main">
    <%
        ...
        System.out.println(c.getNome() + c.getIdCategoria());
    ...
    <%for(Discussione d : discussioni){
        ...
        if(d.getCorpo().length() > 85){
            corpo = corpo.substring(1,80) + ". . .";
        }

    %>
    <a class="post" href="${pageContext.request.contextPath}/pages/post?id=<%= d.getIdDiscussione()%>">
        <div class="intestazione-post">
            ...
            <div class="testo-intestazione-post">
                <p class="nome-utente"><%= nome%></p>
                <p class="data-intestazione-post"><%= d.getDataOra()%></p>
            </div>
        </div>
        <div class="corpo-post">
            <p class="titolo-corpo-post"><%= d.getTitolo()%></p>
            <p class="descrizione-corpo-post"><%= corpo%></p>
        </div>
    </a>
    <%}%>
</div>
